 ...
 * @since 26.03.14
 */
public class DockerAccessObjectWithHcClientSwarm implements DockerAccess {
 ...
 //   private final SwarmHttpClientDelegate swarmDelegate;
     ...
     */
    public DockerAccessObjectWithHcClientSwarm(String apiVersion,
                                    String baseUrl,
                                    String certPath,
                                    int maxConnections,
    ...
    public void communicateWithSwarm(String url, String body, HcChunkedResponseHandlerWrapper handler, int status,
    	...
    	for (int i = 0; i <= retries; i++) {
            try {
                ...
                if (isRetryableErrorCode(e.getStatusCode()) && i != retries) {
                    log.warn("failed to initialize Swarm Network at [{}], retrying...", url);
                } else {
                    throw e;
                }
            }
